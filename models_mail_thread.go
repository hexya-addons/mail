...
user_id, alias)]
        ...
        `,
func(rs m.MailThreadSet, message interface{}, message_dict interface{}, model interface{}, thread_id interface{}, custom_values interface{})  {
...
//        email_to_localpart = (tools.email_split(email_to) or [''])[
//            0].split('@', 1)[0].lower()
//        rcpt_tos = ','.join([
...
//        rcpt_tos_localparts = [e.split('@')[0].lower()
...
h.MailThread().Methods().MessageRouteProcess().DeclareMethod(
`MessageRouteProcess`,
func(rs m.MailThreadSet, message interface{}, message_dict interface{}, routes interface{})  {
...
//                    message_dict.pop('parent_id', None)
//                    thread_id = MessageModel.message_new(
...
//                Model = self.env['mail.thread'].with_context(
//                    thread_model=model)
//            internal = message_dict.pop('internal', False)
//            new_msg = Model.browse(thread_id).message_post(
...
h.MailThread().Methods().MessageProcess().DeclareMethod(
...
provided, this
               ...
               headers.
        `,
func(rs m.MailThreadSet, model interface{}, message interface{}, custom_values interface{}, save_original interface{}, strip_attachments interface{}, thread_id interface{})  {
...
h.MailThread().Methods().MessageNew().DeclareMethod(
...
coming from the message.
           :param dict context: if a ``thread_model`` value is present
                                ...
                                to create (instead of the
current model).
           :rtype: int
           :return: the id of the newly created thread object
        `,
func(rs m.MailThreadSet, msg_dict interface{}, custom_values interface{})  {
//        data = {}
...
h.MailThread().Methods().MessageUpdate().DeclareMethod(
`Called by ``message_process`` when a new message is received
...
to update records
        ...
        `,
func(rs m.MailThreadSet, msg_dict interface{}, update_vals interface{})  {
//        if update_vals:
//            self.write(update_vals)
//        return True
})
h.MailThread().Methods().MessageReceiveBounce().DeclareMethod(
...
func(rs m.MailThreadSet, email interface{}, partner interface{}, mail_id interface{})  {
//        if 'message_bounce' in self._fields:
//            for record in self:
//                record.message_bounce = record.message_bounce + 1
})
h.MailThread().Methods().MessageExtractPayloadPostprocess().DeclareMethod(
...
method
        located in tools. `,
func(rs m.MailThreadSet, message interface{}, body interface{}, attachments interface{})  {
//        if not body:
...
h.MailThread().Methods().MessageExtractPayload().DeclareMethod(
`Extract body as HTML and attachments from the mail message`,
func(rs m.MailThreadSet, message interface{}, save_original interface{})  {
//        attachments = []
//        body = u''
//        if save_original:
...
h.MailThread().Methods().MessageParse().DeclareMethod(
...
h.MailThread().Methods().MessageAddSuggestedRecipient().DeclareMethod(
...
func(rs m.MailThreadSet, result interface{}, partner interface{}, email interface{}, reason interface{})  {
//        self.ensure_one()
...
h.MailThread().Methods().MessageGetSuggestedRecipients().DeclareMethod(
...
h.MailThread().Methods().FindPartnerFromEmails().DeclareMethod(
...
be used as partner emails but it could be the case due to some
        ...
        `,
func(rs m.MailThreadSet, emails interface{}, res_model interface{}, res_id interface{}, check_followers interface{}, force_create interface{}, exclude_aliases interface{})  {
...
//                    # if no match with addr-spec, attempt substring match within name-addr pair
//                    partners = Partner.search(
//                        [('email', 'ilike', email_brackets)], limit=1)
//                partner_id = partners.id
...
h.MailThread().Methods().MessagePartnerInfoFromEmails().DeclareMethod(
...
func(rs m.MailThreadSet, emails interface{}, link_mail interface{})  {
//        self.ensure_one()
...
h.MailThread().Methods().MessagePreprocessAttachments().DeclareMethod(
...
ids, not in tomany command form
        :param str attach_model: the model of the attachments
parent record
        :param integer attach_res_id: the id of the attachments
parent record
        `,
func(rs m.MailThreadSet, attachments interface{}, attachment_ids interface{}, attach_model interface{}, attach_res_id interface{})  {
//        return self._message_post_process_attachments(attachments, attachment_ids, {'model': attach_model, 'res_id': attach_res_id})
})
h.MailThread().Methods().MessagePostProcessAttachments().DeclareMethod(
`MessagePostProcessAttachments`,
func(rs m.MailThreadSet, attachments interface{}, attachment_ids interface{}, message_data interface{})  {
...
h.MailThread().Methods().MessagePost().DeclareMethod(
...
raw HTML that will
            ...
            :param str content_subtype:: if plaintext:
...
be set by Chatter.
        ...
        `,
func(rs m.MailThreadSet, body interface{}, subject interface{}, message_type interface{}, subtype interface{}, parent_id interface{}, attachments interface{}, content_subtype interface{})  {
...
//                partner_to_subscribe = filter(lambda item: item in self._context.get(
...
//            messages = MailMessage.search(['&', ('res_id', '=', self.ids[0]), (
//                'model', '=', model), ('message_type', '=', 'email')], order="id ASC", limit=1)
//            if not messages:
...
h.MailThread().Methods().MessagePostWithView().DeclareMethod(
...
when templates
        handle ir ui views. `,
func(rs m.MailThreadSet, views_or_xmlid interface{})  {
...
h.MailThread().Methods().MessagePostWithTemplate().DeclareMethod(
...
woaerd (which inherit from mail.message)
        `,
func(rs m.MailThreadSet, template_id interface{})  {
...
//            default_template_id=template_id).create(kwargs)
//        if template_id:
...
h.MailThread().Methods().MessageSubscribeUsers().DeclareMethod(
` Wrapper on message_subscribe, using users. If user_ids is not
            provided, subscribe uid instead. `,
func(rs m.MailThreadSet, user_ids interface{}, subtype_ids interface{})  {
//        if user_ids is None:
//            user_ids = [self._uid]
//        return self.message_subscribe(self.env['res.users'].browse(user_ids).mapped('partner_id').ids, subtype_ids=subtype_ids)
})
h.MailThread().Methods().MessageSubscribe().DeclareMethod(
` Add partners to the records followers. `,
func(rs m.MailThreadSet, partner_ids interface{}, channel_ids interface{}, subtype_ids interface{}, force interface{})  {
...
//        gen, part = self.env['mail.followers']._add_follower_command(
//            self._name, self.ids, partner_data, channel_data, force=force)
...
h.MailThread().Methods().MessageUnsubscribeUsers().DeclareMethod(
` Wrapper on message_subscribe, using users. If user_ids is not
            provided, unsubscribe uid instead. `,
func(rs m.MailThreadSet, user_ids interface{})  {
//        if user_ids is None:
//            user_ids = [self._uid]
//        partner_ids = [
//            user.partner_id.id for user in self.env['res.users'].browse(user_ids)]
//        return self.message_unsubscribe(partner_ids)
})
h.MailThread().Methods().MessageUnsubscribe().DeclareMethod(
` Remove partners from the records followers. `,
func(rs m.MailThreadSet, partner_ids interface{}, channel_ids interface{})  {
//        if not partner_ids and not channel_ids:
//            return True
...
h.MailThread().Methods().MessageGetAutoSubscribeFields().DeclareMethod(
` Returns the list of relational fields linking to res.users that should
            trigger an auto subscribe. The default list
checks for the fields
            - called 'user_id'
            - linking to res.users
            - with track_visibility set
            In OpenERP V7, this is sufficent for all major
addon such as opportunity,
            project, issue, recruitment, sale.
            Override this method if a custom behavior is
needed about fields
            that automatically subscribe users.
        `,
func(rs m.MailThreadSet, updated_fields interface{}, auto_follow_fields interface{})  {
...
h.MailThread().Methods().MessageAutoSubscribeNotify().DeclareMethod(
` Notify newly subscribed followers of the last posted message.
            :param partner_ids : the list of partner to
add as needaction partner of the last message
                                 (This excludes the current partner)
        `,
func(rs m.MailThreadSet, partner_ids interface{})  {
//        if not partner_ids:
//            return
...
h.MailThread().Methods().MessageAutoSubscribe().DeclareMethod(
` Handle auto subscription. Two methods for auto subscription exist:

         - tracked res.users relational fields, such as
user_id fields. Those fields
           must be relation fields toward a res.users record,
and must have the
           track_visilibity attribute set.
         - using subtypes parent relationship: check if
the current model being
           modified has an header record (such as a project
for tasks) whose followers
           can be added as followers of the current records.
Example of structure
        ...
        :param dict values: updated values; if None, the
first record will be browsed
                            to get the values. Added after
releasing 7.0, therefore
                            not merged with updated_fields argumment.
        `,
func(rs m.MailThreadSet, updated_fields interface{}, values interface{})  {
...
//                        new_partners.setdefault(
//                            header_follower.partner_id.id, set()).add(new_subtype.id)
//                    else:
//                        new_channels.setdefault(
//                            header_follower.channel_id.id, set()).add(new_subtype.id)
//        user_ids = [values[name]
//                    for name in user_field_lst if values.get(name)]
//        user_pids = [user.partner_id.id for user in self.env['res.users'].sudo().browse(
//            user_ids) if user.partner_id.active]
//        for partner_id in user_pids:
//            new_partners.setdefault(partner_id, None)
//        for pid, subtypes in new_partners.items():
//            subtypes = list(subtypes) if subtypes is not None else None
//            self.message_subscribe(
//                partner_ids=[pid], subtype_ids=subtypes, force=(subtypes != None))
//        for cid, subtypes in new_channels.items():
//            subtypes = list(subtypes) if subtypes is not None else None
//            self.message_subscribe(
//                channel_ids=[cid], subtype_ids=subtypes, force=(subtypes != None))
//        user_pids = [user_pid for user_pid in user_pids if user_pid !=
//                     self.env.user.partner_id.id]
//        self._message_auto_subscribe_notify(user_pids)
//        return True
})
h.MailThread().Methods().MessageChangeThread().DeclareMethod(
`
...
of my_lead to my_project_issue
        `,
func(rs m.MailThreadSet, new_thread interface{})  {
//        self.ensure_one()
...
h.MailThread().Methods().MessageMassMailingEnabled().DeclareMethod(
`MessageMassMailingEnabled`,
func(rs m.MailThreadSet)  {
