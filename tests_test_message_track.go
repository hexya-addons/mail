...
func TestMessageTrack(self interface{}) {
	...
	//        self.group_pigs.sudo(self.user_employee).write(
	//            {'name': 'supername', 'public': 'private'})
	...
	//        self.assertEqual(set(last_msg.tracking_value_ids.mapped(
	//            'field')), set(['name', 'public']))
	//        self.assertEqual(set(last_msg.tracking_value_ids.mapped(
	//            'field_desc')), set(['Name', 'Privacy']))
	//        self.assertEqual(set(last_msg.tracking_value_ids.mapped(
	//            'old_value_char')), set(['my_name', 'Selected group of users']))
	//        self.assertEqual(set(last_msg.tracking_value_ids.mapped(
	//            'new_value_char')), set(['supername', 'Invited people only']))
	//        self.group_pigs.sudo(self.user_employee).write(
	//            {'public': 'public', 'group_public_id': group_system.id})
	//        self.group_pigs.invalidate_cache()
	...
	//        self.assertEqual(set(last_msg.tracking_value_ids.mapped(
	//            'field')), set(['group_public_id', 'public', 'name']))
	//        self.assertEqual(set(last_msg.tracking_value_ids.mapped(
	//            'field_desc')), set(['Authorized Group', 'Privacy', 'Name']))
	//        self.assertEqual(set(last_msg.tracking_value_ids.mapped('old_value_char')), set(
	//            [group_user.name_get()[0][1], 'Invited people only', 'supername']))
	//        self.assertEqual(set(last_msg.tracking_value_ids.mapped('new_value_char')), set(
	//            [group_system.name_get()[0][1], 'Everyone', 'supername']))
	//        self.assertEqual(set(last_msg.tracking_value_ids.mapped(
	//            'old_value_integer')), set([0, group_user.id]))
	//        self.assertEqual(set(last_msg.tracking_value_ids.mapped(
	//            'new_value_integer')), set([0, group_system.id]))
}
