...
func SetUp(self interface{}) {
	//        super(TestMailFollowers, self).setUp()
	//        Subtype = self.env['mail.message.subtype']
	//        self.mt_mg_def = Subtype.create(
	//            {'name': 'mt_mg_def', 'default': True, 'res_model': 'mail.channel'})
	//        self.mt_cl_def = Subtype.create(
	//            {'name': 'mt_cl_def', 'default': True, 'res_model': 'crm.lead'})
	//        self.mt_al_def = Subtype.create(
	//            {'name': 'mt_al_def', 'default': True, 'res_model': False})
	//        self.mt_mg_nodef = Subtype.create(
	//            {'name': 'mt_mg_nodef', 'default': False, 'res_model': 'mail.channel'})
	//        self.mt_al_nodef = Subtype.create(
	//            {'name': 'mt_al_nodef', 'default': False, 'res_model': False})
	//        self.default_group_subtypes = Subtype.search(
	//            [('default', '=', True), '|', ('res_model', '=', 'mail.channel'), ('res_model', '=', False)])
}
func TestM2oCommandNew(self interface{}) {
...
func TestM2oCommandUpdateSelective(self interface{}) {
	//        test_channel = self.env['mail.channel'].create({'name': 'Test'})
	//        groups = self.group_pigs | self.group_public
	//        self.env['mail.followers'].create(
	//            {'partner_id': self.user_employee.partner_id.id, 'res_model': 'mail.channel', 'res_id': self.group_pigs.id})
	//        generic, specific = self.env['mail.followers']._add_follower_command(
	//            'mail.channel', groups.ids,
	//            {self.user_employee.partner_id.id: [self.mt_mg_nodef.id]},
	...
	//        self.assertEqual(set(generic[0][2]['subtype_ids'][0][2]), set(
	//            self.default_group_subtypes.ids))
	//        self.assertEqual(specific.keys(), [self.group_public.id])
	//        self.assertEqual(specific[self.group_public.id]
	//                         [0][2]['res_model'], 'mail.channel')
	//        self.assertEqual(specific[self.group_public.id][0][2]
	//                         ['partner_id'], self.user_employee.partner_id.id)
	//        self.assertEqual(set(specific[self.group_public.id][0][2]['subtype_ids'][0][2]), set(
	//            [self.mt_mg_nodef.id]))
}
func TestMessageIsFollower(self interface{}) {
	//        qty_followed_before = len(self.group_pigs.sudo(
	//            self.user_employee).search([('message_is_follower', '=', True)]))
	//        self.assertFalse(self.group_pigs.sudo(
	//            self.user_employee).message_is_follower)
	//        self.group_pigs.message_subscribe_users(
	//            user_ids=[self.user_employee.id])
	//        qty_followed_after = len(self.group_pigs.sudo(
	//            self.user_employee).search([('message_is_follower', '=', True)]))
	//        self.assertTrue(self.group_pigs.sudo(
	//            self.user_employee).message_is_follower)
	//        self.assertEqual(qty_followed_before + 1, qty_followed_after)
}
func TestFollowersSubtypesDefault(self interface{}) {
	//        self.group_pigs.message_subscribe_users(
	//            user_ids=[self.user_employee.id])
	//        self.assertEqual(self.group_pigs.message_follower_ids.mapped(
	//            'partner_id'), self.user_employee.partner_id)
	//        self.assertEqual(self.group_pigs.message_follower_ids.mapped(
...
func TestFollowersSubtypesDefaultInternal(self interface{}) {
	//        mt_mg_def_int = self.env['mail.message.subtype'].create(
	//            {'name': 'mt_mg_def', 'default': True, 'res_model': 'mail.channel', 'internal': True})
	//        self.group_pigs.message_subscribe_users(
	//            user_ids=[self.user_employee.id])
	//        follower = self.env['mail.followers'].search([
	//            ('res_model', '=', 'mail.channel'),
	...
	//                         self.default_group_subtypes | mt_mg_def_int)
...
func TestFollowersSubtypesSpecified(self interface{}) {
	//        self.group_pigs.sudo(self.user_employee).message_subscribe_users(
...
func TestFollowersMultipleSubscription(self interface{}) {
	//        self.group_pigs.sudo(self.user_employee).message_subscribe_users(
	//            subtype_ids=[self.mt_mg_nodef.id])
	//        self.assertEqual(self.group_pigs.message_follower_ids.mapped(
	//            'partner_id'), self.user_employee.partner_id)
