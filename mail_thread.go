...
// import base64//import datetime//import dateutil//import email//import hashlib//import hmac//import lxml//import logging//import pytz//import re//import socket//import time//try:
//    from xmlrpc import client as xmlrpclib
// except ImportError:
//    import xmlrpclib//_logger = logging.getLogger(__name__)
var fields_MailThread = map[string]models.FieldDefinition{
	"MessageIsFollower": fields.Boolean{
		String:  "Is Follower",
		Compute: h.MailThread().Methods().ComputeIsFollower(),
		// search='_search_is_follower'
	},

	"MessageFollowers": fields.One2Many{
		RelationModel: h.MailFollowers(),
		ReverseFK:     "",
		String:        "Followers",
		Filter:        q.MailFollowers().ResModel().Equals("MailThread")},

	"MessagePartners": fields.Many2Many{
	...
	"MessageChannels": fields.Many2Many{
	...
	"MessageUnread": fields.Boolean{
		String:  "Unread Messages",
		Compute: h.MailThread().Methods().GetMessageUnread(),
		Help:    "If checked new messages require your attention."},

	"MessageUnreadCounter": fields.Integer{
		String:  "Unread Messages Counter",
		Compute: h.MailThread().Methods().GetMessageUnread(),
		Help:    "Number of unread messages"},

	"MessageNeedaction": fields.Boolean{
		String:  "Action Needed",
		Compute: h.MailThread().Methods().GetMessageNeedaction(),
		// search='_search_message_needaction'
		Help: "If checked, new messages require your attention."},

	"MessageNeedactionCounter": fields.Integer{
		String:  "Number of Actions",
		Compute: h.MailThread().Methods().GetMessageNeedaction(),
		Help:    "Number of messages which requires an action"},

	"MessageHasError": fields.Boolean{
		String:  "Message Delivery error",
		Compute: h.MailThread().Methods().ComputeMessageHasError(),
		// search='_search_message_has_error'
		Help: "If checked, some messages have a delivery error."},

	"MessageHasErrorCounter": fields.Integer{
		String:  "Number of error",
		Compute: h.MailThread().Methods().ComputeMessageHasError(),
		Help:    "Number of messages with delivery error"},

	"MessageAttachmentCount": fields.Integer{
		String:  "Attachment Count",
		Compute: h.MailThread().Methods().ComputeMessageAttachmentCount()},

	"MessageMainAttachment": fields.Many2One{
...
// GetFollowers
func mailThread_GetFollowers(rs m.MailThreadSet) m.MailThreadData {
...
//
func mailThread_SearchFollowerPartners(rs m.MailThreadSet, operator interface{}, operand interface{}) {
...
// message_is_followers
//
func mailThread_SearchFollowerChannels(rs m.MailThreadSet, operator interface{}, operand interface{}) {
	//        assert operator != "not in", "Do not search message_follower_ids with 'not in'"
	//        followers = self.env['mail.followers'].sudo().search([
	//            ('res_model', '=', self._name),
...
// ComputeIsFollower
func mailThread_ComputeIsFollower(rs m.MailThreadSet) m.MailThreadData {
	//        followers = self.env['mail.followers'].sudo().search([
	//            ('res_model', '=', self._name),
	//            ('res_id', 'in', self.ids),
...
// SearchIsFollower
func mailThread_SearchIsFollower(rs m.MailThreadSet, operator interface{}, operand interface{}) {
	//        followers = self.env['mail.followers'].sudo().search([
	//            ('res_model', '=', self._name),
	//            ('partner_id', '=', self.env.user.partner_id.id),
	//        ])
...
// GetMessageUnread
func mailThread_GetMessageUnread(rs m.MailThreadSet) m.MailThreadData {
...
func mailThread_GetMessageNeedaction(rs m.MailThreadSet) m.MailThreadData {
	//        res = dict((res_id, 0) for res_id in self.ids)
	//        if not res:
	//            return
	//        self._cr.execute(""" SELECT msg.res_id FROM mail_message msg
...
func mailThread_SearchMessageNeedaction(rs m.MailThreadSet, operator interface{}, operand interface{}) {
	//        return [('message_ids.needaction', operator, operand)]
}

// ComputeMessageHasError
func mailThread_ComputeMessageHasError(rs m.MailThreadSet) m.MailThreadData {
...
// SearchMessageHasError
func mailThread_SearchMessageHasError(rs m.MailThreadSet, operator interface{}, operand interface{}) {
	//        return ['&', ('message_ids.has_error', operator, operand), ('message_ids.author_id', '=', self.env.user.partner_id.id)]
}

// ComputeMessageAttachmentCount
func mailThread_ComputeMessageAttachmentCount(rs m.MailThreadSet) m.MailThreadData {
...
//
func mailThread_Create(rs m.MailThreadSet, vals_list models.RecordData) {
	...
	//                message_follower_ids += [(0, 0, fol_vals) for fol_vals in self.env['mail.followers']._add_default_followers(
	//                    self._name, [], self.env.user.partner_id.ids, customer_ids=[])[0][0]]
...
// Write
func mailThread_Write(rs m.MailThreadSet, values models.RecordData) {
	//        if self._context.get('tracking_disable'):
	...
	//        if tracked_fields:
	//            track_self.with_context(clean_context(self._context)).message_track(
...
func mailThread_Unlink(rs m.MailThreadSet) {
	//        if not self:
	//            return True
...
// CopyData
func mailThread_CopyData(rs m.MailThreadSet, defaultName models.RecordData) {
	//        return super(MailThread, self.with_context(mail_notrack=True)).copy_data(default=default)
}

//  Override of BaseModel.get_empty_list_help() to generate an help message
//         that adds alias information.
func mailThread_GetEmptyListHelp(rs m.MailThreadSet, help interface{}) {
...
// FieldsViewGet
func mailThread_FieldsViewGet(rs m.MailThreadSet, view_id webtypes.FieldsViewGetParams, view_type interface{}, toolbar interface{}, submenu interface{}) {
...
//         cases, for example when sending emails on chatter
// or sending mass
//         mailings. It replaces
//
//          * href of links (mailto will not match the regex)
//          * src of images (base64 hardcoded data will not
// match the regex)
//          * styling using url like background-image: url
//
//         It is done using regex because it is shorten than
// using an html parser
//         to create a potentially complex soupe and hope
// to have a result that
//         has not been harmed.
//
func mailThread_ReplaceLocalLinks(rs m.MailThreadSet, html interface{}, base_url interface{}) {
	//        if not html:
	//            return html
	//        html = ustr(html)
	...
	//                _sub_relative2absolute.base_url = self.env["ir.config_parameter"].sudo(
	...
	//        html = re.sub(
	//            r"""(<[^>]+\bstyle="[^"]+\burl\('?)(/[^/'][^'")]+)""", _sub_relative2absolute, html)
	//        return html
}
...
//
func mailThread_GarbageCollectAttachments(rs m.MailThreadSet) {
...
// such as issues.
func mailThread_CheckMailMessageAccess(rs m.MailThreadSet, res_ids interface{}, operation interface{}, model_name interface{}) {
	//        if model_name:
...
//
//         Transfer the list of the mail thread messages from
// an model to another
//
...
//                     will transfer the context of the thread
// of my_lead to my_project_task
//
func mailThread_MessageChangeThread(rs m.MailThreadSet, new_thread interface{}) {
	//        self.ensure_one()
	//        subtype_comment = self.env['ir.model.data'].xmlid_to_res_id(
	//            'mail.mt_comment')
	...
	//        msg_not_comment.write(
	//            {"res_id": new_thread.id, "model": new_thread._name, "subtype_id": None})
	//        return True
}
...
// description, containing on_change fields
//
func mailThread_GetTrackedFields(rs m.MailThreadSet, updated_fields interface{}) {
	//        tracked_fields = []
...
//
func mailThread_TrackSubtype(rs m.MailThreadSet, init_values interface{}) {
	//        return False
}

// TrackTemplate
func mailThread_TrackTemplate(rs m.MailThreadSet, tracking interface{}) {
	//        return dict()
}

// MessageTrackPostTemplate
func mailThread_MessageTrackPostTemplate(rs m.MailThreadSet, tracking interface{}) {
...
func mailThread_MessageTrackGetChanges(rs m.MailThreadSet, tracked_fields interface{}, initial_values interface{}) {
	//        result = dict()
	//        for record in self:
...
func mailThread_MessageTrack(rs m.MailThreadSet, tracked_fields interface{}, initial interface{}) {
	//        self.ensure_one()
...
func mailThread_MessageTrack(rs m.MailThreadSet, tracked_fields interface{}, initial_values interface{}) {
...
// NotifyEncodeLink
func mailThread_NotifyEncodeLink(rs m.MailThreadSet, base_link interface{}, params interface{}) {
	...
	//                      token.encode('utf-8'), hashlib.sha1).hexdigest()
	//        return hm
}

// NotifyGetActionLink
func mailThread_NotifyGetActionLink(rs m.MailThreadSet, link_type interface{}) {
...
//
func mailThread_NotifyGetGroups(rs m.MailThreadSet, message interface{}, groups interface{}) {
	//        return groups
}

//  Classify recipients to be notified of a message in groups to have
//         specific rendering depending on their group. For
// example users could
//         have access to buttons customers should not have
// in their emails.
//
...
//
func mailThread_NotifyClassifyRecipients(rs m.MailThreadSet, message interface{}, recipient_data interface{}) {
	//        result = {}
...
//         inheritance and allowing to call model-specific
// implementation in a one liner.
//         This method should not be overridden.
func mailThread_NotifyClassifyRecipientsOnRecords(rs m.MailThreadSet, message interface{}, recipient_data interface{}, records interface{}) {
...
//         on documents. Documents are either given by self
// it this method is called
//         as a true multi method on a record set or can be
// given using records to
//         have a generic implementation available for all models.
//
//         Reply-to is formatted like "MyCompany MyDocument
// <reply.to@domain>".
//         Heuristic it the following:
//
//          * search for specific aliases as they always have
// priority; it is limited
//            to aliases linked to documents (like project
// alias for task for example);
//          * use catchall address;
//          * use default;
//
//         This method works on documents
//
//          * as a true multi method for models inheriting
// from mail.thread; call
//            ``records._notify_get_reply_to(...)``;
//          * as a generic implementation if records are given;
// call ``MailThread.
//            _notify_get_reply_to(records=records, ...)``;
//          * as a generic implementation is self is a void
// mail.thread record set
//            meaning catchall is computed; call ``MailThread._notify_get_reply_to
//            (records=None)``;
//
//         Tweak this method on a specific model by overriding
// if it inherits from
//         mail.thread. An example would be tasks taking their
// reply-to alias from
//         their project.
//
//         :param default: default email if no alias or catchall is found;
//         :param records: record_set if self if a generic
// mail.thread record allowing
//           generic implementation of finding reply-to;
//         :param company: used to compute company name part
// of the from name; provide
//           it if already known, otherwise fall back on user company;
//         :param doc_names: dict(res_id, doc_name) used to
// compute doc name part of
//           the from name; provide it if already known to
// avoid queries, otherwise
//           name_get on document will be performed;
//
//         :return result: dictionary. Keys are record IDs
// and value is formatted
//           like an email "Company_name Document_name <reply_to@email>"/
//
func mailThread_NotifyGetReplyTo(rs m.MailThreadSet, defaultName interface{}, records interface{}, company interface{}, doc_names interface{}) {
	...
	//        alias_domain = self.env['ir.config_parameter'].sudo(
	//        ).get_param("mail.catchall.domain")
	...
	//                if not doc_names:
	//                    doc_names = dict((rec.id, rec.display_name)
	//                                     for rec in _records)
	//
	...
	//                    result_email.setdefault(alias.alias_parent_thread_id, '%s@%s' % (
	//                        alias.alias_name, alias_domain))
	//
	//            # left ids: use catchall
	//            left_ids = set(_res_ids) - set(result_email)
	//            if left_ids:
	//                catchall = self.env['ir.config_parameter'].sudo(
	...
	//                name = '%s%s%s' % (company_name, ' ' if doc_names.get(
	//                    res_id) else '', doc_names.get(res_id, ''))
...
//  Generic wrapper on ``_notify_get_reply_to`` checking mail.thread
// inheritance
//         and allowing to call model-specific implementation
// in a one liner. This
//         method should not be overridden.
func mailThread_NotifyGetReplyToOnRecords(rs m.MailThreadSet, defaultName string, records m.MailThreadSet,
	company m.CompanySet, docNames map[int64]string) map[int64]string {
...
//
func mailThread_NotifySpecificEmailValues(rs m.MailThreadSet, message interface{}) {
	//        if not self:
	//            return {}
	//        self.ensure_one()
	//        return {'headers': repr({
...
//  Generic wrapper on ``_notify_specific_email_values`` checking
// mail.thread
//         inheritance and allowing to call model-specific
// implementation in a one liner.
//         This method should not be overridden.
func mailThread_NotifySpecificEmailValuesOnRecords(rs m.MailThreadSet, message interface{}, records interface{}) {
...
//         mail.mail. Basic method just set the recipient
// partners as mail_mail
//         recipients. Override to generate other mail values
// like email_to or
//         email_cc.
//
//         :param message: mail.message record being notified by email
...
//
func mailThread_NotifyEmailRecipients(rs m.MailThreadSet, message interface{}, recipient_ids interface{}) {
	//        return {
...
// implementation in a one liner.
//         This method should not be overridden.
func mailThread_NotifyEmailRecipientsOnRecords(rs m.MailThreadSet, message interface{}, recipient_ids interface{}, records interface{}) {
...
// NotifyCustomizeRecipients can be overridden to modify msgVals before returning it
func mailThread_NotifyCustomizeRecipients(rs m.MailThreadSet, message m.MailMessageSet, msgVals m.MailMessageData, data mailtypes.RecipientsData) m.MailMessageData {
	return msgVals
}
...
func mailThread_MessageFindPartners(rs m.MailThreadSet, message interface{}, header_fields interface{}) {
...
func mailThread_RoutingWarn(rs m.MailThreadSet, error_message interface{}, warn_suffix interface{}, message_id interface{}, route interface{}, raise_exception interface{}) {
...
func mailThread_RoutingCreateBounceEmail(rs m.MailThreadSet, email_from interface{}, body_html interface{}, message interface{}) {
...
//                               discussions
//
func mailThread_MessageRouteVerify(rs m.MailThreadSet, message interface{}, message_dict interface{}, route interface{}, update_author interface{}, assert_model interface{}, create_fallback interface{}, allow_private interface{}, drop_alias interface{}) {
	...
	//                check_result = obj._alias_check_contact(
	//                    message, message_dict, alias)
	//            else:
	//                check_result = self.env['mail.alias.mixin']._alias_check_contact_on_record(
	//                    obj, message, message_dict, alias)
	//            if check_result is not True:
	//                self._routing_warn(_('alias %s: %s') % (alias.alias_name, check_result.get(
	//                    'error_message', _('unknown error'))), _('skipping'), message_id, route, False)
	//                self._routing_create_bounce_email(email_from, check_result.get(
	//                    'error_template', _generic_bounce_body_html), message)
	//                return False
...
// pair and
//            ignore custom_value as no creation will take
// place (6.1 compatibility)
//          * look for a mail.alias entry matching the message
// recipients and use the
//            corresponding model, thread_id, custom_values
// and user_id. This could
//            lead to a thread update or creation depending on the alias
//          * fallback on provided ``model``, ``thread_id``
// and ``custom_values``
//          * raise an exception as no route has been found
//
//         :param string message: an email.message instance
//         :param dict message_dict: dictionary holding parsed
// message variables
//         :param string model: the fallback model to use
// if the message does not match
//             any of the currently configured mail aliases
// (may be None if a matching
//             alias is supposed to be present)
//         :type dict custom_values: optional dictionary of
// default field values
//             to pass to ``message_new`` if a new record
// needs to be created.
//             Ignored if the thread record already exists,
// and also if a matching
//             mail.alias was found (aliases define their own defaults)
//         :param int thread_id: optional ID of the record/thread
// from ``model`` to
//             which this mail should be attached. Only used
// if the message does not
//             reply to an existing thread and does not match
// any mail alias.
//         :return: list of routes [(model, thread_id, custom_values,
// user_id, alias)]
//
//         :raises: ValueError, TypeError
//
func mailThread_MessageRoute(rs m.MailThreadSet, message interface{}, message_dict interface{}, model interface{}, thread_id interface{}, custom_values interface{}) {
	//        if not isinstance(message, Message):
	//            raise TypeError(
	//                'message must be an email.message.Message at this point')
	//        MailMessage = self.env['mail.message']
	//        Alias, dest_aliases = self.env['mail.alias'], self.env['mail.alias']
	//        catchall_alias = self.env['ir.config_parameter'].sudo(
	//        ).get_param("mail.catchall.alias")
	...
	//            thread_references)
	//        email_from = tools.decode_message_header(message, 'From')
	//        email_from_localpart = (tools.email_split(email_from) or [''])[
	//            0].split('@', 1)[0].lower()
	...
	//                "%s\+(\d+)-?([\w.]+)?-?(\d+)?" % re.escape(bounce_alias), re.UNICODE)
	...
	//
	//                email_part = next((part for part in message.walk(
	//                ) if part.get_content_type() == 'message/rfc822'), None)
	//                dsn_part = next((part for part in message.walk(
	...
	//                        dsn, 'Final-Recipient')
	//                    partner_address = final_recipient_data.split(';', 1)[
	//                        1].strip()
	//                    if partner_address:
	//                        partners = partners.sudo().search(
	//                            [('email', '=', partner_address)])
	//                        for partner in partners:
	//                            partner.message_receive_bounce(
	//                                partner_address, partner, mail_id=bounced_mail_id)
	//
	...
	//            return []
	//        msg_references = [ref for ref in tools.mail_header_msgid_re.findall(
	...
	//                    route = (alias.alias_model_id.model, alias.alias_force_thread_id, safe_eval(
...
// MessageRouteProcess
func mailThread_MessageRouteProcess(rs m.MailThreadSet, message interface{}, message_dict interface{}, routes interface{}) {
	...
	//                    message_dict.pop('parent_id', None)
	//                    thread = MessageModel.message_new(
	...
	//
	//            post_params = dict(
	//                subtype=subtype, partner_ids=partner_ids, **message_dict)
...
//             is called with the new message (if the thread
// record did not exist)
//             or its ``message_update`` method (if it did).
//
//             There is a special case where the target model
// is False: a reply
//             to a private message. In this case, we skip
// the message_new /
//             message_update step, to just post a new message
// using mail_thread
//             message_post.
//
//            :param string model: the fallback model to use if the message
//                does not match any of the currently configured
// mail aliases
//                (may be None if a matching alias is supposed
// to be present)
...
//                 Ignored if the thread record already exists,
// and also if a
//                 matching mail.alias was found (aliases
// define their own defaults)
//            :param bool save_original: whether to keep a
// copy of the original
//                 email source attached to the message after
// it is imported.
//            :param bool strip_attachments: whether to strip
// all attachments
//                 before processing the message, in order
// to save some space.
//            :param int thread_id: optional ID of the record/thread
// from ``model``
//                to which this mail should be attached. When
// provided, this
//                overrides the automatic detection based on the message
//                headers.
//
func mailThread_MessageProcess(rs m.MailThreadSet, model interface{}, message interface{}, custom_values interface{}, save_original interface{}, strip_attachments interface{}, thread_id interface{}) {
...
//            an existing thread.
//            The default behavior is to create a new record
// of the corresponding
//            model (based on some very basic info extracted
// from the message).
//            Additional behavior may be implemented by overriding
// this method.
//
//            :param dict msg_dict: a map containing the email details and
//                                  attachments. See ``message_process``
// and
...
//
func mailThread_MessageNew(rs m.MailThreadSet, msg_dict interface{}, custom_values interface{}) {
	//        data = {}
...
// Called by ``message_process`` when a new message is received
//            for an existing thread. The default behavior
// is to update the record
...
//                                ``mail.message.parse()`` for details.
//            :param dict update_vals: a dict containing values
// to update records
//                               given their ids; if the dict is None or is
//                               void, no write operation is performed.
//
func mailThread_MessageUpdate(rs m.MailThreadSet, msg_dict interface{}, update_vals interface{}) {
	//        if update_vals:
...
//         Mail Returned to Sender) is received for an existing
// thread. The default
//         behavior is to check is an integer  ``message_bounce``
// column exists.
//         If it is the case, its content is incremented.
//
//         :param mail_id: ID of the sent email that bounced.
// It may not exist anymore
//                         but it could be usefull if the
// information was kept. This is
//                         used notably in mass mailing.
//         :param RecordSet partner: partner matching the
// bounced email address, if any
//         :param string email: email that caused the bounce
func mailThread_MessageReceiveBounce(rs m.MailThreadSet, email interface{}, partner interface{}, mail_id interface{}) {
...
//         extracted from the email. Note that this processing
// is specific to the
//         mail module, and should not contain security or
// generic html cleaning.
//         Indeed those aspects should be covered by the html_sanitize
// method
//         located in tools.
func mailThread_MessageExtractPayloadPostprocess(rs m.MailThreadSet, message interface{}, body interface{}, attachments interface{}) {
	//        if not body:
...
func mailThread_MessageExtractPayload(rs m.MailThreadSet, message interface{}, save_original interface{}) {
	//        attachments = []
	//        body = u''
	//        if save_original:
	...
	//                    attachments.append(self._Attachment(
	//                        filename or 'attachment', part.get_payload(decode=True), {}))
	//        body, attachments = self._message_extract_payload_postprocess(
...
//                     }
//
func mailThread_MessageParse(rs m.MailThreadSet, message interface{}, save_original interface{}) {
	//        msg_dict = {
	//            'message_type': 'email',
	//        }
	//        if not isinstance(message, Message):
	...
	//            msg_dict['subject'] = tools.decode_smtp_header(
	//                message.get('Subject'))
	...
	//                [('message_id', '=', tools.decode_smtp_header(message['In-Reply-To'].strip()))], limit=1)
	//            if parent_ids:
	...
	//            msg_list = tools.mail_header_msgid_re.findall(
	//                tools.decode_smtp_header(message['References']))
	//            parent_ids = self.env['mail.message'].search(
	//                [('message_id', 'in', [x.strip() for x in msg_list])], limit=1)
	//            if parent_ids:
	//                msg_dict['parent_id'] = parent_ids.id
	//                msg_dict['internal'] = parent_ids.subtype_id and parent_ids.subtype_id.internal or False
	//        msg_dict['body'], msg_dict['attachments'] = self._message_extract_payload(
	//            message, save_original=save_original)
	//        return msg_dict
}

// MessageGetDefaultRecipients
func mailThread_MessageGetDefaultRecipients(rs m.MailThreadSet, res_model interface{}, res_ids interface{}) {
...
// or partner_name, reason
func mailThread_MessageAddSuggestedRecipient(rs m.MailThreadSet, result interface{}, partner interface{}, email interface{}, reason interface{}) {
	//        self.ensure_one()
	...
	//                    [partner_info['partner_id']])[0]
	//        if email and email in [val[1] for val in result[self.ids[0]]]:
	//            return result
	//        if partner and partner in self.message_partner_ids:  # recipient already in the followers -> skip
	//            return result
	//        if partner and partner.id in [val[0] for val in result[self.ids[0]]]:
	//            return result
	//        if partner and partner.email:  # complete profile: id, name <email>
	//            result[self.ids[0]].append(
	//                (partner.id, '%s<%s>' % (partner.name, partner.email), reason))
	//        elif partner:  # incomplete profile: id, name
	//            result[self.ids[0]].append(
	//                (partner.id, '%s' % (partner.name), reason))
...
//         tuple (partner_id, partner_name, reason), to be
// managed by Chatter.
func mailThread_MessageGetSuggestedRecipients(rs m.MailThreadSet) {
...
// SearchOnUser
func mailThread_SearchOnUser(rs m.MailThreadSet, email_address interface{}, extra_domain interface{}) {
	//        Users = self.env['res.users'].sudo()
	//        partners = Users.search(
	//            [('email', '=ilike', email_address)], limit=1).mapped('partner_id')
	//        if not partners:
	//            # if no match with addr-spec, attempt substring match within name-addr pair
	//            email_brackets = "<%s>" % email_address
	//            partners = Users.search(
	//                [('email', 'ilike', email_brackets)], limit=1).mapped('partner_id')
	//        return partners.id
}

// SearchOnPartner
func mailThread_SearchOnPartner(rs m.MailThreadSet, email_address interface{}, extra_domain interface{}) {
	//        Partner = self.env['res.partner'].sudo()
	//        partners = Partner.search(
	//            [('email', '=ilike', email_address)] + extra_domain, limit=1)
	//        if not partners:
	//            # if no match with addr-spec, attempt substring match within name-addr pair
	//            email_brackets = "<%s>" % email_address
	//            partners = Partner.search(
	//                [('email', 'ilike', email_brackets)] + extra_domain, limit=1)
	//        return partners.id
}
...
//             :param string model: model to fetch related
// record; by default self
//                 is used.
//             :param boolean check_followers: check in document followers
//             :param boolean force_create: create a new partner
// if not found
//             :param boolean exclude_aliases: do not try
// to find a partner that could match an alias. Normally aliases
//                                             should not
// be used as partner emails but it could be the case due to some
//                                             strange manipulation
//
func mailThread_FindPartnerFromEmails(rs m.MailThreadSet, emails interface{}, res_model interface{}, res_id interface{}, check_followers interface{}, force_create interface{}, exclude_aliases interface{}) {
	...
	//            partner_id = next(
	//                (partner.id for partner in followers if partner.email == email_address), False)
...
//             new_partner_ids. The return value is non conventional
// because
//             it is meant to be used by the mail widget.
//
//             :return dict: partner_ids and new_partner_ids
func mailThread_MessagePartnerInfoFromEmails(rs m.MailThreadSet, emails interface{}, link_mail interface{}) {
	//        self.ensure_one()
	...
	//            partner_info = {'full_name': email_address,
...
//  Preprocess attachments for mail_thread.message_post()
// or mail_mail.create().
//
//         :param list attachments: list of attachment tuples
// in the form ``(name,content)``,
//                                  where content is NOT base64 encoded
//         :param list attachment_ids: a list of attachment
// ids, not in tomany command form
//         :param dict message_data: model: the model of the
// attachments parent record,
//           res_id: the id of the attachments parent record
//
func mailThread_MessagePostProcessAttachments(rs m.MailThreadSet, attachments interface{}, attachment_ids interface{}, message_data interface{}) {
	...
	//                    if not attachment:
	//                        attachment = fname_mapping.get(
	//                            node.get('data-filename'), '')
	//                    if attachment:
	//                        attachment.generate_access_token()
	//                        node.set('src', '/web/image/%s?access_token=%s' %
...
//             mail.message ID.
//             :param int thread_id: thread ID to post into,
// or list with one ID;
//                 if False/0, mail.message model will also be set as False
//             :param str body: body of the message, usually
// raw HTML that will
//                 be sanitized
//             :param str type: see mail_message.message_type field
//             :param int parent_id: handle reply to a previous
// message by adding the
//                 parent partners to the message in case
// of private discussion
//             :param tuple(str,str) attachments or list id:
// list of attachment tuples in the form
//                 ``(name,content)``, where content is NOT base64 encoded
//             Extra keyword arguments will be used as default
// column values for the
//             new mail.message record. Special cases:
//                 - attachment_ids: supposed not attached
// to any document; attach them
//                     to the related document. Should only
// be set by Chatter.
//             :return int: ID of newly created mail.message
//
func mailThread_MessagePost(rs m.MailThreadSet, body interface{}, subject interface{}, message_type interface{}, subtype interface{}, parent_id interface{}, attachments interface{}, notif_layout interface{}, add_sign interface{}, model_description interface{}, mail_auto_delete interface{}) {
	...
	//            self.ensure_one()
	//            model = kwargs.get(
	//                'model', False) if self._name == 'mail.thread' else self._name
	...
	//                partner_to_subscribe = [p for p in partner_ids if p in self._context.get(
	...
	//                ['&', ('res_id', '=', self.ids[0]), ('model', '=', model)], order="id ASC", limit=1)
	...
	//        self._message_post_after_hook(
	//            new_message, values, model_description=model_description, mail_auto_delete=mail_auto_delete)
	//        return new_message
}

//  Hook to add custom behavior after having posted the message. Both
//         message and computed value are given, to try to
// lessen query count by
//         using already-computed values instead of having
// to rebrowse things.
func mailThread_MessagePostAfterHook(rs m.MailThreadSet, message interface{}, msg_vals interface{}, model_description interface{}, mail_auto_delete interface{}) {
	...
	//            message._notify(
	//                self, msg_vals,
	//                force_send=self.env.context.get(
...
//         render using the ir.qweb engine. This method is
// stand alone, because
//         there is nothing in template and composer that allows to handle
//         views in batch. This method should probably disappear
// when templates
//         handle ir ui views.
func mailThread_MessagePostWithView(rs m.MailThreadSet, views_or_xmlid interface{}) {
	...
	//            rendered_template = views.render(
...
// to render to create the body of the message
//             :param **kwargs : parameter to create a mail.compose.message
// woaerd (which inherit from mail.message)
//
func mailThread_MessagePostWithTemplate(rs m.MailThreadSet, template_id interface{}) {
	...
	//            custom_layout=notif_layout).create(kwargs)
	//        if template_id:
...
//         any document. It pushes notifications on inbox
// or by email depending
//         on the user configuration, like other notifications.
func mailThread_MessageNotify(rs m.MailThreadSet, partner_ids interface{}, body interface{}, subject interface{}) {
	...
	//                _("Unable to notify message, please configure the sender's email address."))
...
//         any notification and pre-computes some values to
// have a short code
//         as optimized as possible. This method is private
// as it does not check
//         access rights and perform the message creation
// as sudo to speedup
//         the log process. This method should be called within
// methods where
//         access rights are already granted to avoid privilege
// escalation.
func mailThread_MessageLog(rs m.MailThreadSet, body interface{}, subject interface{}, message_type interface{}) {
	//        if len(self.ids) > 1:
	//            raise exceptions.Warning(
	//                _('Invalid record set: should be called as model (without records) or on single-record recordset'))
	//        kw_author = kwargs.pop('author_id', False)
...
//         to perform access rights checks before calling
// ``_message_subscribe``.
func mailThread_MessageSubscribe(rs m.MailThreadSet, partner_ids interface{}, channel_ids interface{}, subtype_ids interface{}) {
	...
	//                [('id', 'in', partner_ids), ('active', '=', True)]).ids
...
//         partners and channels, given their IDs, as followers
// of all records
//         contained in the record set.
//
//         If subtypes are given existing followers are erased
// with new subtypes.
//         If default one have to be computed only missing
// followers will be added
//         with default subtypes matching the record set model.
//
//         This private method does not specifically check
// for access right. Use
//         ``message_subscribe`` public API when not sure
// about access rights.
//
//         :param customer_ids: see ``_insert_followers``
func mailThread_MessageSubscribe(rs m.MailThreadSet, partner_ids interface{}, channel_ids interface{}, subtype_ids interface{}, customer_ids interface{}) {
	//        if not self:
	//            return True
	//        if not subtype_ids:
...
func mailThread_MessageUnsubscribe(rs m.MailThreadSet, partner_ids interface{}, channel_ids interface{}) {
...
//             notification is required)), aka partners and
// their subtype and possible notification to send
...
//         documents. This is done using relational fields
// linking to res.users
//         with track_visibility set. Since OpenERP v7 it
// is considered as being
//         responsible for the document and therefore standard
// behavior is to
//         subscribe the user and send him a notification.
//
//         Override this method to change that behavior and/or
// to add people to
...
//
func mailThread_MessageAutoSubscribeFollowers(rs m.MailThreadSet, updated_values interface{}, default_subtype_ids interface{}) {
	//        fnames = []
	//        for name, field in self._fields.items():
	...
	//                    fnames.append(name)
	//        new_subscriptions = []
	//        user_ids = [updated_values[fname]
	//                    for fname in fnames if updated_values[fname]]
	//        if user_ids:
	//            new_pids = self.env['res.partner'].sudo().search(
	//                [('user_ids', 'in', user_ids), ('active', '=', True)]).ids
	//            for new_pid in new_pids:
	//                new_subscriptions.append(
	//                    (new_pid, default_subtype_ids, 'mail.message_user_assigned' if new_pid != self.env.user.partner_id.id else False))
	//        return new_subscriptions
}

//  Notify new followers, using a template to render the content of the
//         notification message. Notifications pushed are
// done using the standard
//         notification mechanism in mail.thread. It is either
// inbox either email
//         depending on the partner state: no user (email,
// customer), share user
...
//
func mailThread_MessageAutoSubscribeNotify(rs m.MailThreadSet, partner_ids interface{}, template interface{}) {
	...
	//                'model_description': model_description,
	//            }
	//            assignation_msg = view.render(
	//                values, engine='ir.qweb', minimal_qcontext=True)
	//            assignation_msg = self.env['mail.thread']._replace_local_links(
	//                assignation_msg)
...
//         main mechanisms
//
//          * using subtypes parent relationship. For example
// following a parent record
//            (i.e. project) with subtypes linked to child
// records (i.e. task). See
//            mail.message.subtype ``_get_auto_subscription_subtypes``;
//          * calling _message_auto_subscribe_notify that
// returns a list of partner
//            to subscribe, as well as data about the subtypes
// and notification
//            to send. Base behavior is to subscribe responsible
// and notify them;
//
//         Adding application-specific auto subscription should
// be done by overriding
//         ``_message_auto_subscribe_followers``. It should
// return structured data
//         for new partner to subscribe, with subtypes and
// eventual notification
//         to perform. See that method for more details.
//
//         :param updated_values: values modifying the record
// trigerring auto subscription
//
func mailThread_MessageAutoSubscribe(rs m.MailThreadSet, updatedValues m.MailThreadData) bool {
	//        if not self:
	//            return True
	//        new_partners, new_channels = dict(), dict()
	//        updated_relation = dict()
	//        all_ids, def_ids, int_ids, parent, relation = self.env['mail.message.subtype']._get_auto_subscription_subtypes(
	//            self._name)
	...
	//        udpated_fields = [fname for fnames in updated_relation.values(
	//        ) for fname in fnames if updated_values.get(fname)]

	if rs.IsEmpty() {
		return true
	}
	// fetch auto subscription subtypes data
	all, def, internal, parent, relation := h.MailMessageSubtype().NewSet(rs.Env()).GetAutoSubscriptionSubtypes(rs.ModelName())

	// check effectively modified relation field
	updatedRelation := make(map[string]models.FieldNames)
	for resModel, fNames := range relation {
		for _, fName := range fNames {
			if !updatedValues.Has(fName) {
				continue
			}
			updatedRelation[resModel] = append(updatedRelation[resModel], fName)
		}
	}
	var updatedFields models.FieldNames
	for _, fieldNames := range updatedRelation {
		updatedFields = append(updatedFields, fieldNames...)
	}

	if len(updatedFields) > 0 {
		h.MailFollowers().NewSet(rs.Env()).GetSubscriptionData()
	}

	//        if udpated_fields:
	//            doc_data = [(model, [updated_values[fname] for fname in fnames])
	//                        for model, fnames in updated_relation.items()]
	//            res = self.env['mail.followers']._get_subscription_data(
	//                doc_data, None, None, include_pshare=True)
	...
	//                if pid:
	//                    new_partners[pid] = (set(sids) & set(
	//                        all_ids)) - set(int_ids) if pshare else set(sids) & set(all_ids)
	//                if cid:
	//                    new_channels[cid] = (
	//                        set(sids) & set(all_ids)) - set(int_ids)
	//        notify_data = dict()
	...
	//            self.with_context(lang=lang)._message_auto_subscribe_notify(
	//                pids, template)
	//        return True
}
func init() {
	models.NewModel("MailThread")
	h.MailThread().AddFields(fields_MailThread)
	h.MailThread().NewMethod("GetFollowers", mailThread_GetFollowers)
	h.MailThread().NewMethod("SearchFollowerPartners", mailThread_SearchFollowerPartners)
	h.MailThread().NewMethod("SearchFollowerChannels", mailThread_SearchFollowerChannels)
	h.MailThread().NewMethod("ComputeIsFollower", mailThread_ComputeIsFollower)
	h.MailThread().NewMethod("SearchIsFollower", mailThread_SearchIsFollower)
	h.MailThread().NewMethod("GetMessageUnread", mailThread_GetMessageUnread)
	h.MailThread().NewMethod("GetMessageNeedaction", mailThread_GetMessageNeedaction)
	h.MailThread().NewMethod("SearchMessageNeedaction", mailThread_SearchMessageNeedaction)
	h.MailThread().NewMethod("ComputeMessageHasError", mailThread_ComputeMessageHasError)
	h.MailThread().NewMethod("SearchMessageHasError", mailThread_SearchMessageHasError)
	h.MailThread().NewMethod("ComputeMessageAttachmentCount", mailThread_ComputeMessageAttachmentCount)
	h.MailThread().Methods().Create().Extend(mailThread_Create)
	h.MailThread().Methods().Write().Extend(mailThread_Write)
	h.MailThread().Methods().Unlink().Extend(mailThread_Unlink)
	h.MailThread().Methods().CopyData().Extend(mailThread_CopyData)
	h.MailThread().NewMethod("GetEmptyListHelp", mailThread_GetEmptyListHelp)
	h.MailThread().Methods().FieldsViewGet().Extend(mailThread_FieldsViewGet)
	h.MailThread().NewMethod("ReplaceLocalLinks", mailThread_ReplaceLocalLinks)
	h.MailThread().NewMethod("GarbageCollectAttachments", mailThread_GarbageCollectAttachments)
	h.MailThread().NewMethod("CheckMailMessageAccess", mailThread_CheckMailMessageAccess)
	h.MailThread().NewMethod("MessageChangeThread", mailThread_MessageChangeThread)
	h.MailThread().NewMethod("GetTrackedFields", mailThread_GetTrackedFields)
	h.MailThread().NewMethod("TrackSubtype", mailThread_TrackSubtype)
	h.MailThread().NewMethod("TrackTemplate", mailThread_TrackTemplate)
	h.MailThread().NewMethod("MessageTrackPostTemplate", mailThread_MessageTrackPostTemplate)
	h.MailThread().NewMethod("MessageTrackGetChanges", mailThread_MessageTrackGetChanges)
	h.MailThread().NewMethod("MessageTrack", mailThread_MessageTrack)
	h.MailThread().NewMethod("MessageTrack", mailThread_MessageTrack)
	h.MailThread().NewMethod("NotifyEncodeLink", mailThread_NotifyEncodeLink)
	h.MailThread().NewMethod("NotifyGetActionLink", mailThread_NotifyGetActionLink)
	h.MailThread().NewMethod("NotifyGetGroups", mailThread_NotifyGetGroups)
	h.MailThread().NewMethod("NotifyClassifyRecipients", mailThread_NotifyClassifyRecipients)
	h.MailThread().NewMethod("NotifyClassifyRecipientsOnRecords", mailThread_NotifyClassifyRecipientsOnRecords)
	h.MailThread().NewMethod("NotifyGetReplyTo", mailThread_NotifyGetReplyTo)
	h.MailThread().NewMethod("NotifyGetReplyToOnRecords", mailThread_NotifyGetReplyToOnRecords)
	h.MailThread().NewMethod("NotifySpecificEmailValues", mailThread_NotifySpecificEmailValues)
	h.MailThread().NewMethod("NotifySpecificEmailValuesOnRecords", mailThread_NotifySpecificEmailValuesOnRecords)
	h.MailThread().NewMethod("NotifyEmailRecipients", mailThread_NotifyEmailRecipients)
	h.MailThread().NewMethod("NotifyEmailRecipientsOnRecords", mailThread_NotifyEmailRecipientsOnRecords)
	h.MailThread().NewMethod("NotifyCustomizeRecipients", mailThread_NotifyCustomizeRecipients)
	h.MailThread().NewMethod("MessageFindPartners", mailThread_MessageFindPartners)
	h.MailThread().NewMethod("RoutingWarn", mailThread_RoutingWarn)
	h.MailThread().NewMethod("RoutingCreateBounceEmail", mailThread_RoutingCreateBounceEmail)
	h.MailThread().NewMethod("MessageRouteVerify", mailThread_MessageRouteVerify)
	h.MailThread().NewMethod("MessageRoute", mailThread_MessageRoute)
	h.MailThread().NewMethod("MessageRouteProcess", mailThread_MessageRouteProcess)
	h.MailThread().NewMethod("MessageProcess", mailThread_MessageProcess)
	h.MailThread().NewMethod("MessageNew", mailThread_MessageNew)
	h.MailThread().NewMethod("MessageUpdate", mailThread_MessageUpdate)
	h.MailThread().NewMethod("MessageReceiveBounce", mailThread_MessageReceiveBounce)
	h.MailThread().NewMethod("MessageExtractPayloadPostprocess", mailThread_MessageExtractPayloadPostprocess)
	h.MailThread().NewMethod("MessageExtractPayload", mailThread_MessageExtractPayload)
	h.MailThread().NewMethod("MessageParse", mailThread_MessageParse)
	h.MailThread().NewMethod("MessageGetDefaultRecipients", mailThread_MessageGetDefaultRecipients)
	h.MailThread().NewMethod("MessageAddSuggestedRecipient", mailThread_MessageAddSuggestedRecipient)
	h.MailThread().NewMethod("MessageGetSuggestedRecipients", mailThread_MessageGetSuggestedRecipients)
	h.MailThread().NewMethod("SearchOnUser", mailThread_SearchOnUser)
	h.MailThread().NewMethod("SearchOnPartner", mailThread_SearchOnPartner)
	h.MailThread().NewMethod("FindPartnerFromEmails", mailThread_FindPartnerFromEmails)
	h.MailThread().NewMethod("MessagePartnerInfoFromEmails", mailThread_MessagePartnerInfoFromEmails)
	h.MailThread().NewMethod("MessagePostProcessAttachments", mailThread_MessagePostProcessAttachments)
	h.MailThread().NewMethod("MessagePost", mailThread_MessagePost)
	h.MailThread().NewMethod("MessagePostAfterHook", mailThread_MessagePostAfterHook)
	h.MailThread().NewMethod("MessagePostWithView", mailThread_MessagePostWithView)
	h.MailThread().NewMethod("MessagePostWithTemplate", mailThread_MessagePostWithTemplate)
	h.MailThread().NewMethod("MessageNotify", mailThread_MessageNotify)
	h.MailThread().NewMethod("MessageLog", mailThread_MessageLog)
	h.MailThread().NewMethod("MessageSubscribe", mailThread_MessageSubscribe)
	h.MailThread().NewMethod("MessageSubscribe", mailThread_MessageSubscribe)
	h.MailThread().NewMethod("MessageUnsubscribe", mailThread_MessageUnsubscribe)
	h.MailThread().NewMethod("MessageAutoSubscribeFollowers", mailThread_MessageAutoSubscribeFollowers)
	h.MailThread().NewMethod("MessageAutoSubscribeNotify", mailThread_MessageAutoSubscribeNotify)
	h.MailThread().NewMethod("MessageAutoSubscribe", mailThread_MessageAutoSubscribe)
